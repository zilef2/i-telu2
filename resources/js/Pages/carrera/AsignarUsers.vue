...
const data = reactive({
    ...
    showText700: window.innerWidth > 740
})

const form = useForm({
    selectedId: [],
    carreraid: props.carrera?.id,
})

watch(() => _.cloneDeep(data.params), debounce(() => {
    let params = pickBy(data.params)
    router.get(route("carrera.AsignarUsers", props.carrera.id), params, {
        replace: true,
        preserveState: true,
        preserveScroll: true,
    })
...
const inscribirSubmit = () => {
    form.post(route('carrera.SubmitAsignarUsers'), {
        preserveScroll: true,
        onSuccess: () => {
            // emit("close")
        ...
        <section class="text-gray-600 body-font">
            <div class="container px-5 py-1 mx-auto">
                <div class="flex flex-col text-center w-full mb-12">
                    <h1 class="sm:text-3xl text-2xl font-medium title-font mb-2 text-gray-900">Matricular a la carrera</h1>
                    <p v-if="data.showText700" class="w-full mx-auto leading-relaxed text-base">
                        Estudiantes de la <b>{{ props.universidad.nombre }}</b> que no pertenecen a <b>{{
                            props.carrera?.nombre }}</b>
                    </p>
                ...
                <div class="flex flex-wrap -m-2">
                    <div v-if="props.usuariosPorInscribir.length > 0" v-for="(user, index) in props.usuariosPorInscribir"
                        :key="index" class="p-1 xl:w-1/4 lg:w-1/2 md:w-1/2 w-full border-gray-200 border rounded-lg">
                    ...
                    <div v-else class="p-1 w-full border-gray-200 border rounded-lg text-center">
                        <p class="w-full mx-auto leading-relaxed text-base text-red-700">
                ...
                <div v-if="props.inscritos.length > 0" class="flex flex-wrap my-8">
                    <div class="p-4 xl:w-1/3 sm:w-1/2 w-full">
                        <h2
                            class="font-medium title-font tracking-widest text-gray-900 mb-4 text-xl text-center sm:text-left">
                            Inscritos</h2>
                        <nav class="flex flex-col sm:items-start sm:text-left text-center items-center -mb-1 space-y-2.5">
                            <p v-for="(user, index) in props.inscritos" :key="index">
                                <span
