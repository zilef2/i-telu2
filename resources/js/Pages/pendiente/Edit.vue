...
const update = () => {
    form.put(route('pendiente.update', props.pendiente?.id), {
        preserveScroll: true,
        onSuccess: () => {
            emit("close")
            form.reset()
...
watchEffect(() => {
    if (props.show) {
        form.errors = {}
        form.nombre = props.pendiente?.nombre
        form.descripcion = props.pendiente?.descripcion
        form.universidad_id = props.pendiente?.universidad_id
        form.enum = props.pendiente?.enum
        form.codigo = props.pendiente?.codigo
    }
})
</script>
