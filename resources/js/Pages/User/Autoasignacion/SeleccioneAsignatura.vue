        ...
        <div class="space-y-4">
            <div v-if="materias.data.length !== 0" class="bg-white py-6 sm:py-8 lg:py-12">
                <div  class="mx-auto max-w-screen-2xl px-4 md:px-8">
                        <h2 class="my-4 text-center text-2xl font-bold text-gray-800 lg:text-3xl ">Adquirir asignaturas</h2>
                        <p class="my-6 text-center text-gray-600">Haga clic en el nombre de la materia para seleccionarla.</p>

                    <div class="grid grid-cols-2 gap-4 rounded-lg md:grid-cols-3 lg:gap-6">
                        <div v-for="(materia, index) in materias.data" :key="index">
                            <div class="min-w-full inline-flex h-16 items-center justify-center rounded-lg p-4 text-gray-400 sm:h-32 bg-gray-100 hover:text-blue-200"
                                 :class="{'bg-green-300': form.materias.includes(materia.id)}"
                                 @click="AddMateria(materia.id)">
                                    <!--  validar que no haya repetidos-->
                                <div class="mx-auto max-w-screen-2xl px-4 md:px-8">
                                    <div class="flex flex-col items-center">
                                        <p class="mb-4 text-sm font-semibold text-indigo-500 md:text-base">
                                          Carrera: {{materia.laCarrera}}
                                        </p>
                                        <h1 class="mb-2 text-center text-xl font-bold text-gray-800 md:text-3xl">
                                          {{ materia.nombre }}
                                        </h1>
