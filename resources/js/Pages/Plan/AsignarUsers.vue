...
watch(() => _.cloneDeep(data.params), debounce(() => {
    ...
    router.get(route("Plan.AsignarUsers", props.Plan.id), params, {
        replace: true,
        preserveState: true,
        preserveScroll: true,
    })
...
const inscribirSubmit = () => {
    form.post(route('Plan.SubmitAsignarUsers'), {
        preserveScroll: true,
        onSuccess: () => {
            // emit("close")
...
const toEraseIdSubmit = () => {
    form.post(route('Plan.toEraseId'), {
        preserveScroll: true,
        onSuccess: () => {
            // emit("close")
            // data.mensajeForm = "listo!"
        ...
        <section class="text-gray-600 body-font">
            <div class="py-1 grid justify-items-center mx-6">
                <div class="flex flex-col text-center w-full mb-12">
                    <h1 class="sm:text-3xl text-2xl font-medium title-font mb-2 text-gray-900">Matricular a la Plan
                    </h1>
                    <p class="w-full mx-auto leading-relaxed text-base">
                        Estudiantes que no pertenecen a <b>{{ Plan?.nombre }}</b>
                    </p>
                    <TextInput v-if="props.numberPermissions > 1" v-model="data.params.search" type="text"
                        class="my-4 mx-auto block w-4/6 md:w-3/6 lg:w-2/6 rounded-lg"
                        placeholder="Nombre, identificacion" />
                    ...
                    <div class="container px-5 pb-2 mx-auto flex flex-wrap flex-col">
                        ...
                        <div class="flex flex-col text-center w-full">
                            ...
                            <div v-if="data.mostrarSection === 1" class="flex flex-wrap -m-2">
                                ...
                                <div v-else class="p-1 w-full border-gray-200 border rounded-lg text-center">
                                    <p class="w-full mx-auto leading-relaxed text-base text-red-400">
                                        No hay mas estudiantes que no pertenescan a la {{ Plan?.nombre }}
                                    </p>
                                </div>
                            </div>
                            ...
                            <div v-if="data.mostrarSection === 2" class="flex flex-wrap -m-2">
                                ...
                                <div v-else class="p-1 w-full border-gray-200 border rounded-lg text-center">
                                    <p class="w-full mx-auto leading-relaxed text-base text-red-400">
                                        {{ Plan?.nombre }} no tiene estudiantes
                                    </p>
                                </div>
                            </div>
                            ...
                            <div v-if="data.mostrarSection === 4" class="flex flex-wrap -m-2">
                                ...
                                <div v-else class="p-1 w-full border-gray-200 border rounded-lg text-center">
                                    <p class="w-full mx-auto leading-relaxed text-base text-red-400">
                                        {{ Plan?.nombre }} no tiene Profesores
                                    </p>
                                </div>
                            </div>
                        </div>
